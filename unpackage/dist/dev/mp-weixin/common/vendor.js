...
function renderSlot(name, props = {}, key2) {
  ...
  if (!parent && !isMounted) {
    onMounted(() => {
      renderSlot(name, props, key2);
    }, instance);
    return;
  }
  const invoker = findScopedSlotInvoker(vueIds, instance);
  if (invoker) {
    invoker(name, props, key2);
  }
}
...
const r = (name, props, key2) => renderSlot(name, props, key2);
...
function initRuntimeSocketService() {
  const hosts = "192.168.10.17,127.0.0.1";
  const port = "8090";
  const id = "mp-weixin_YjUrXu";
